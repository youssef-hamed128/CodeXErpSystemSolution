...
namespace CodeXErpSystem.Controllers
{
    ...
    public class SalesReturnController : Controller
    {
        ...
        public SalesReturnController(IInvoiceService invoiceService, IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        public async Task<IActionResult> Index()
        {
            var invoices = await _unitOfWork.GetRepository<Invoice>().FindAsync(
                i => i.Type == InvoiceType.SalesReturn, includeProperties: "Customer", orderBy: q => q.OrderByDescending(x => x.Id));
        ...
        public async Task<IActionResult> Create()
        {
            ...
            var model = new InvoiceCreateViewModel { Type = InvoiceType.SalesReturn, Date = DateTime.UtcNow };
            model.InvoiceNumber = await _invoiceService.GenerateInvoiceNumberAsync(InvoiceType.SalesReturn);
            return View(model);
        }
        ...
        {
            model.Type = InvoiceType.SalesReturn;
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    TempData["Success"] = $"تم حفظ مرتجع المبيعات بنجاح. رقم الفاتورة: {result.InvoiceNumber}";
        ...
        public async Task<IActionResult> Print(int id)
        {
            ...
            if (invoice == null || invoice.Type != InvoiceType.SalesReturn)
            {
                return NotFound();
            }
